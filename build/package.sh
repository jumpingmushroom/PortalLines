...
DLL="$ROOT/src/PortalLines/bin/Release/net472/PortalLines.dll"
...
ASM_VERSION=$(grep -oP '(?<=PluginVersion = ")[^"]+' "$ROOT/src/PortalLines/Plugin.cs")
...
echo "==> staging"
rm -rf "$STAGE"
mkdir -p "$STAGE/plugins/PortalLines"
cp "$DLL"                        "$STAGE/plugins/PortalLines/"
...
OUT="$ROOT/dist/PortalLines-$VERSION.zip"
rm -f "$OUT"
( cd "$STAGE" && zip -qr "$OUT" . )
